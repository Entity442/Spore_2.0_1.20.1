...
public class Tentacle extends UtilityEntity {
    ...
    private final IkLegWithHitbox rightFrontLeg;
    private final IkLegWithHitbox leftFrontLeg;
    private final IkLegWithHitbox rightBackLeg;
    private final IkLegWithHitbox leftBackLeg;
    private final IkLegWithHitbox[] legs;
    public Tentacle(EntityType<? extends PathfinderMob> type, Level level) {
        ...
        this.rightFrontLeg = new IkLegWithHitbox(this,new TentaclePart[]{part0, part1, part2}, part2,LEGS.RIGHT_FRONT.getBodySet(),LEGS.RIGHT_FRONT.getOffset()
        ,100,1,4);
        this.leftFrontLeg = new IkLegWithHitbox(this,new TentaclePart[]{part3, part4, part5}, part5,LEGS.LEFT_FRONT.getBodySet(),LEGS.LEFT_FRONT.getOffset()
                ,100,1,4);
        this.rightBackLeg = new IkLegWithHitbox(this,new TentaclePart[]{part6, part7, part8}, part8,LEGS.RIGHT_BACK.getBodySet(),LEGS.RIGHT_BACK.getOffset()
                ,100,1,4);
        this.leftBackLeg = new IkLegWithHitbox(this,new TentaclePart[]{part9, part10, part11}, part11,LEGS.LEFT_BACK.getBodySet(),LEGS.LEFT_BACK.getOffset()
                ,100,1,4);
        legs = new IkLegWithHitbox[]{rightFrontLeg,leftFrontLeg,rightBackLeg,leftBackLeg};
    ...
    public PartEntity<?>[] getRightSegments(){return rightFrontLeg.getHitBoxes();}
    public PartEntity<?>[] getLeftSegments(){return leftFrontLeg.getHitBoxes();}
    public PartEntity<?>[] getRightBackSegments(){return rightBackLeg.getHitBoxes();}
    public PartEntity<?>[] getLeftBackSegments(){return leftBackLeg.getHitBoxes();}
    ...
    public void aiStep() {
        super.aiStep();
        for(IkLegWithHitbox leg : legs){
            if (tickCount % 5 == 0){
                leg.refreshLegStandingPoint();
            }
            leg.applyIK();
            spawnParticlesInPlaces(leg.getSitPosition(),leg.getDefaultBodyOffset(),false);
            spawnParticlesInPlaces(leg.getSitPosition(),leg.getDefaultBodyOffset(),true);
        }
    }
    private void spawnParticlesInPlaces(Vec3 vec3, Vec3 positionOnBody,boolean fire){
        if (fire){
