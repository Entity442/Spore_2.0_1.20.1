...
public class GargoyleRenderer<Type extends Gargoyl> extends BaseInfectedRenderer<Type , gargoyleModel<Type>> {
    private static final ResourceLocation EMPTY =new ResourceLocation("spore:textures/entity/empty.png");
    ...
    private static class ProtectorArmorRenderer <T extends Gargoyl> extends RenderLayer<T, gargoyleModel<T>> {
        ...
        private void renderArmorPart(T entity, EquipmentSlot slot , ModelPart arm, PoseStack stack, MultiBufferSource bufferSource, int packedLight){
            ...
            if (itemStack.getItem() instanceof ArmorItem armorItem){
                if (armorItem instanceof DyeableLeatherItem) {
                    ...
                    renderArmor(arm,stack,bufferSource,packedLight,OverlayTexture.NO_OVERLAY,f,f1,f2,1,this.getArmorResource(entity, itemStack, slot, (String)null),flag,slot);
                } else {
                    renderArmor(arm,stack,bufferSource,packedLight,OverlayTexture.NO_OVERLAY,1,1,1,1,this.getArmorResource(entity, itemStack, slot, (String)null),flag,slot);
                }
                ArmorTrim.getTrim(entity.level().registryAccess(), itemStack).ifPresent((p_289638_) -> {
                    this.renderTrim(armorItem.getMaterial(), stack, bufferSource, packedLight, p_289638_, arm, flag);
                });
            }
        }
        private void renderArmor(ModelPart parts, PoseStack stack, MultiBufferSource bufferSource, int packedLight, int packedOverlay, float red, float green, float blue, float alpha, ResourceLocation location, boolean glint, EquipmentSlot slot){
            ...
            this.getParentModel().Gargoyle.render(stack, consumer, packedLight, packedOverlay, red, green, blue, alpha);
            if (glint){
                this.getParentModel().Gargoyle.render(stack, bufferSource.getBuffer(RenderType.entityGlint()), packedLight, packedOverlay, red, green, blue, alpha);
            }
        ...
        private void renderTrim(ArmorMaterial material, PoseStack stack, MultiBufferSource source, int light, ArmorTrim armorTrim, ModelPart parts, boolean flag) {
            ...
            this.getParentModel().Gargoyle.getAllParts().forEach(modelPart -> {setInvisible(modelPart,parts);});
            this.getParentModel().Gargoyle.render(stack, vertexconsumer, light, OverlayTexture.NO_OVERLAY, 1f, 1f, 1f, 1f);
        }
